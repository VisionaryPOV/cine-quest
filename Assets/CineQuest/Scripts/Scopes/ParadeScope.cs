// Cine Quest — RGB Parade scope.

using UnityEngine;

namespace CineQuest.Scopes
{
    public sealed class ParadeScope : MonoBehaviour
    {
        [SerializeField] int columns = 128;
        ...
        public void Process(RenderTexture source, ComputeShader cs)
        {
            ...
            int total = columns * binCount * 3;
            cs.SetBuffer(_kClear, "_Bins", _bins);
            cs.Dispatch(_kClear, Mathf.CeilToInt(total / 64f), 1, 1);
        ...
        void EnsureResources()
        {
            ...
            int outW = columns * 3;
            if (_outRt == null || _outRt.width != outW || _outRt.height != binCount)
            {
                if (_outRt != null) { _outRt.Release(); Destroy(_outRt); }
                _outRt = new RenderTexture(outW, binCount, 0, RenderTextureFormat.ARGB32)
                {
                    name = "ParadeRT",
