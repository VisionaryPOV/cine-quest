// Cine Quest — RGB/Luma histogram (CPU path on analysis RT for simplicity + stretch completeness).
// Prefer GPU for production tuning; this keeps dependencies light and accurate enough for assist.
...
namespace CineQuest.Scopes
{
    public sealed class HistogramScope : MonoBehaviour
    {
        ...
        {
            if (!isActiveAndEnabled || source == null || _readbackPending) return;

            int w = source.width;
            int h = source.height;
            // Downsample further for CPU histogram
            int rw = Mathf.Min(w, 160);
            int rh = Mathf.Min(h, 90);
            ...
            // Async would be better; sync readback only on reduced RT and not every frame (ScopeManager rate-limits).
            ...
            for (int i = 0; i < px.Length; i += step)
            {
                ...
                _histG[c.g]++;
                _histB[c.b]++;
                int y = (c.r * 54 + c.g * 183 + c.b * 19) >> 8; // ~Rec.709 8-bit
            ...
            for (int x = 0; x < bins; x++)
            {
                ...
                for (int y = 0; y < texH; y++)
                {
                    ...
                    // Emphasize luma as white overlay
